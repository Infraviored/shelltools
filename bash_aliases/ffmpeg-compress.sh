...
# ffmpeg-compress-720p  — 720p cap, 30fps, standard GPU encode
...
ffmpeg-compress-720p() {
  local in="$1"
  [ -z "$in" ] && {
    echo "usage: ffmpeg-compress-720p <input-file>"
    return 1
  }
  local out="${in%.*}_compressed_720p.mp4"
  ffmpeg -hide_banner -y -i "$in" \
    -map_metadata 0 \
    -vf "scale='min(1280,iw)':'min(720,ih)':force_original_aspect_ratio=decrease:force_divisible_by=2,fps=30,format=yuv420p" \
    -c:v h264_nvenc -preset p5 -b:v 1000k \
    -c:a aac -b:a 96k \
    -movflags +faststart \
    "$out"
}
